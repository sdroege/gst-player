...
state_changed_cb (GstBus * bus, GstMessage * msg, gpointer user_data)
{
  ...
  if (GST_MESSAGE_SRC (msg) == GST_OBJECT (self->priv->playbin)) {
    ...
    if (old_state == GST_STATE_READY && new_state == GST_STATE_PAUSED
      ...
      g_mutex_lock (&self->priv->lock);
      media_info = gst_player_media_info_create (self);
      g_mutex_unlock (&self->priv->lock);
      emit_media_updated_signal (self, media_info, SIGNAL_MEDIA_INFO_UPDATED);
...
static GstPlayerMediaInfo*
media_info_copy (GstPlayer *self, GstPlayerMediaInfo *media_info,
  gint stream_index, GType type)
{
  ...
  /* create a copy of media_info */
  info = gst_player_media_info_copy (media_info);
  return info;
}
...
tags_changed_cb (GstPlayer *self, gint stream_index, GType type)
{
  ...
  g_mutex_lock (&self->priv->lock);
  media_info = media_info_copy (self, self->priv->media_info,
                  stream_index, type);
  g_mutex_unlock (&self->priv->lock);
