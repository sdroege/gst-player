...
{
  g_return_if_fail (GST_IS_PLAYER_STREAM_INFO (info));
...
{
  g_return_if_fail (GST_IS_PLAYER_AUDIO_INFO (info));
...
gst_player_media_info_copy (GstPlayerMediaInfo* ref)
{
  ...
  g_return_val_if_fail (GST_IS_PLAYER_MEDIA_INFO (ref), NULL);

  info = gst_player_media_info_new (ref->uri);

  for (l = ref->stream_list; l != NULL; l = l->next)
    info->stream_list = g_list_append (info->stream_list,
                  gst_player_stream_info_copy((GstPlayerStreamInfo*)l->data));
